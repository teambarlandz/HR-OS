...
mod regs {
    pub const ACC: u8 = 10; // a0 (x10): Accumulator register
    pub const SCRATCH: u8 = 11; // a1 (x11): Scratch register
}
...
pub unsafe fn compile_and_run(
    ...
    // so native dispatch faults. The ELF is now patched post-link by
    // scripts/patch-riscv-x.py (PT_LOAD RW->RWE), so native ITIM execution
    // is safe on both architectures.
