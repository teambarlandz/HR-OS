...
pub mod lexer {
    ...
    impl<'a> Lexer<'a> {
        ...
        pub fn next_token(&mut self) -> Token<'a> {
            while self.cursor < self.stream.len() && self.stream[self.cursor] == b' ' {
                self.cursor += 1;
            }
            ...
            if b.is_ascii_alphabetic() {
                ...
                while self.cursor < self.stream.len()
                    && self.stream[self.cursor].is_ascii_alphanumeric()
                {
                    self.cursor += 1;
                }
                ...
                let mut v = 0u32;
                while self.cursor < self.stream.len() && self.stream[self.cursor].is_ascii_digit() {
                    v = v * 10 + (self.stream[self.cursor] - b'0') as u32;
                    self.cursor += 1;
                }
                Token::Literal(v)
            } else {
